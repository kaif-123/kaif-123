<h1 align="center">Hi, I'm Kaif 👋</h1>

<p align="center">
  <b>B.Tech CSE (Data Science Minor) · Lovely Professional University</b><br/>
  Full-Stack Developer &amp; AI/ML Enthusiast · Open to SDE Roles
</p>

<p align="center">
  <a href="https://www.linkedin.com/in/md-kaif-5888bb304">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=flat&logo=linkedin&logoColor=white"/>
  </a>
  <a href="https://leetcode.com/u/kaifuu/">
    <img src="https://img.shields.io/badge/LeetCode-FFA116?style=flat&logo=leetcode&logoColor=white"/>
  </a>
  <a href="https://www.hackerrank.com/profile/kaifmd197">
    <img src="https://img.shields.io/badge/HackerRank-2EC866?style=flat&logo=hackerrank&logoColor=white"/>
  </a>
  <img src="https://komarev.com/ghpvc/?username=kaif-123&color=blueviolet&style=flat"/>
</p>

---

## About Me

- 🎓 3rd-year CSE student at **LPU** with a **Data Science minor**
- 🔭 Currently building **LocalFix** — a location-based home services marketplace
- 💡 Passionate about full-stack development, AI/ML, and building real-world products
- 🏆 **250+ LeetCode problems** solved · **3-star HackerRank** in Java
- 📫 Reach me at: kaifmd197@gmail.com**

---

## 🛠️ Tech Stack

**Languages**
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat&logo=openjdk&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=flat&logo=c%2B%2B&logoColor=white)

**Frontend**
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat&logo=css3&logoColor=white)
![React](https://img.shields.io/badge/React-20232A?style=flat&logo=react&logoColor=61DAFB)

**Backend & DB**
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat&logo=nodedotjs&logoColor=white)
![Express](https://img.shields.io/badge/Express-000000?style=flat&logo=express&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=flat&logo=mongodb&logoColor=white)
![Socket.io](https://img.shields.io/badge/Socket.io-010101?style=flat&logo=socketdotio&logoColor=white)

**AI/ML & Tools**
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat&logo=pytorch&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=flat&logo=fastapi&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)

---

## 🚀 Featured Projects

### [LocalFix](https://github.com/kaif-123/localfix) — Home Services Marketplace
> Urban Company-style platform with real-time booking, GPS-based worker matching, and in-app chat

- **Stack:** Node.js · Express · MongoDB · Socket.io · JWT · Razorpay
- 📍 GPS-based worker discovery with geospatial queries
- 🔔 Real-time booking flow via Socket.io (accept/reject/track)
- 💳 Wallet system + Razorpay payment integration
- 🔐 JWT auth, OTP verification, dynamic worker profiles

---

### [AI Image Detector](https://github.com/kaif-123/ai-image-detector-backend) — Real vs AI-Generated
> CNN-based classifier to detect AI-generated images with a clean web interface

- **Stack:** PyTorch · FastAPI · Framer · Railway
- 🧠 Custom-trained CNN model for binary classification
- ⚡ FastAPI backend with REST endpoints, deployed on Railway

---

### [Mira](https://github.com/kaif-123/ai-mira-chatbot) — Gen Z AI Chatbot
> Conversational AI persona powered by Groq API (LLaMA 3.3 70B)

- **Stack:** Groq API · llama-3.3-70b-versatile · JavaScript
- 💬 Custom Gen Z persona with contextual memory
- 🚀 Sub-second response times via Groq's fast inference

---

### [Portfolio Website](https://github.com/kaif-123/portfolio)
> Personal portfolio with neural network canvas background, skills marquee, and project showcase

- **Stack:** HTML · CSS · Vanilla JS · Canvas API
- ✨ Animated neural network background, floating navbar

---

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=kaif-123&show_icons=true&theme=tokyonight&hide_border=true&count_private=true" height="160"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=kaif-123&layout=compact&theme=tokyonight&hide_border=true" height="160"/>
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=kaif-123&theme=tokyonight&hide_border=true"/>
</p>

---

## 🏅 Certifications

- 📜 Deloitte - Data Analytics
- 📜 HackerRank — Java Certification
- 📜 OpenCV University — OpenCV Bootcamp
- 📜 Infosys - Build Generative AI Apps
- 📜 CSE Pathsala - C++ Programming: OOP and DSA
- 📜 IBM - Fundamentals of Network Communication

---

## 🎮 Catch Me Off Guard

```
While (!placed) {
    leetcode.grind();
    build.projects();
    coffee++;
}
```

---

<p align="center">
  <i>⭐ Star a repo if you found it useful — it motivates me to keep building!</i>
</p>
## 🐍 Contribution Snake

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/kaif-123/kaif-123/output/github-contribution-grid-snake-dark.svg"/>
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/kaif-123/kaif-123/output/github-contribution-grid-snake.svg"/>
  <img alt="contribution snake" src="https://raw.githubusercontent.com/kaif-123/kaif-123/output/github-contribution-grid-snake.svg"/>
</picture>
